...
class Article {
  ...
  public static function getPage($conn, $limit, $offset, $only_published = false) {
    ...
    FROM
    (SELECT * FROM article $condition ORDER BY published_at LIMIT :limit OFFSET :offset) AS a
  ...
  public function setImageFile($conn, $filename) {
    ...
    $stmt = $conn->prepare($sql);
    $stmt->bindValue(':image_file', $filename, is_null($filename) ? PDO::PARAM_NULL : PDO::PARAM_STR);
    $stmt->bindValue(':id', $this->id, PDO::PARAM_INT);
    return $stmt->execute();
  }
